...
bool RotaryDeltaStrategy::handleConfig()
{
    // default is probably wrong
    this->probe_radius= THEKERNEL->config->value(leveling_strategy_checksum, rotary_delta_strategy_checksum, radius_checksum)->by_default(-1)->as_number();
    ...
    this->initial_height= THEKERNEL->config->value(leveling_strategy_checksum, rotary_delta_strategy_checksum, initial_height_checksum)->by_default(10)->as_number();
    return true;
}

bool RotaryDeltaStrategy::handleGcode(Gcode *gcode)
{
    if( gcode->has_g) {
        // G code processing
...
static std::tuple<float, float, float, float, float, float> getCoordinates(float radius)
{
    ...
    float t1x = 0.0F, t1y = -radius; // X arm
    float t2x = px, t2y = py; // Y arm
    float t3x = -px, t3y = py; // Z arm
    return std::make_tuple(t1x, t1y, t2x, t2y, t3x, t3y);
}


// Probes the 7 points on a delta can be used for off board calibration
bool RotaryDeltaStrategy::probe_delta_points(Gcode *gcode)
{
    float bedht= findBed();
...
float RotaryDeltaStrategy::findBed()
{
    // home
    zprobe->home();
...
bool RotaryDeltaStrategy::calibrate_delta_endstops(Gcode *gcode)
{
    float target = 0.03F;
    ...
    for (int i = 1; i <= 10; ++i) {
        ...
        if(trimx < -5 || trimy < -5 || trimz < -5) {
            gcode->stream->printf("Trim: {%1.3f, %1.3f, %1.3f}\n", trimx, trimy, trimz);
            gcode->stream->printf("Values less than -5 suggest that something is horribly wrong.\n");
            return false;
        }
        ...
        zprobe->home();
        zprobe->coordinated_move(NAN, NAN, bedht, zprobe->getFastFeedrate(), false); // do a relative move from home to the point above the bed
        ...
        auto mm_fix = std::minmax({trimx, trimy, trimz});
        trimx -= mm_fix.second;
        trimy -= mm_fix.second;
        trimz -= mm_fix.second;
...
bool RotaryDeltaStrategy::calibrate_delta_radius(Gcode *gcode)
{
    float target = 0.03F;
    if(gcode->has_letter('I')) target = gcode->get_value('I'); // override default target
    if(gcode->has_letter('J')) this->probe_radius = gcode->get_value('J'); // override default probe radius
    ...
    for (int i = 1; i <= 10; ++i) {
        ...
        zprobe->home();
        zprobe->coordinated_move(NAN, NAN, bedht, zprobe->getFastFeedrate(), false); // needs to be a relative coordinated move
...
/*
    probe edges to get outer positions, then probe center
    modify the delta z_offset until center and X converge
*/

bool RotaryDeltaStrategy::calibrate_z_offset(Gcode *gcode)
{
    float target = 0.03F;
    if(gcode->has_letter('I')) target = gcode->get_value('I'); // override default target
    if(gcode->has_letter('J')) this->probe_radius = gcode->get_value('J'); // override default probe radius

    gcode->stream->printf("Calibrating delta z offset: target %f, radius %f\n", target, this->probe_radius);
    ...
    gcode->stream->printf("Probe accuracy is ok\n");

    // get current z offset
    float z_offset = 0.0F;
    BaseSolution::arm_options_t options;
    if(THEROBOT->arm_solution->get_optional(options)) {
        z_offset = options['E'];
    ...
    for (int i = 1; i <= 10; ++i) {
        ...
        gcode->stream->printf("T3-%d Z:%1.3f\n", i, dy);
        if(!zprobe->doProbeAt(dz, t3x, t3y)) return false;
        gcode->stream->printf("T4-%d Z:%1.3f\n", i, dz);
        ...
        options['E'] = z_offset;
        THEROBOT->arm_solution->set_optional(options);
        gcode->stream->printf("Setting delta z offset to: %1.4f\n", z_offset);

        zprobe->home();
        zprobe->coordinated_move(NAN, NAN, bedht, zprobe->getFastFeedrate(), false);
    ...
    if(!good) {
        gcode->stream->printf("WARNING: delta z offset did not resolve to within required parameters: %f\n", target);
    }

    return true;
}

bool RotaryDeltaStrategy::set_trim(float x, float y, float z, StreamOutput *stream)
{
    float t[3] {x, y, z};
...
bool RotaryDeltaStrategy::get_trim(float &x, float &y, float &z)
{
    void *returned_data;
